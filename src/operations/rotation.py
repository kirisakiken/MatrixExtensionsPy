...
class RotationOperator:
    ...
    @staticmethod
    def __get_rotation_matrix_2d(degree: float):
        rad = math.radians(degree)
        return np.array([[math.cos(rad), -math.sin(rad)],
                         [math.sin(rad), math.cos(rad)]])

    @staticmethod
    def __get_rotation_matrix_3d(degree: float, axis: AXIS):
        rad = math.radians(degree)
        if axis == AXIS.X:
    ...
    def get_rotation_2d(self, vector_2: Vector2, degree: float) -> Vector2:
        rotation_matrix = self.__get_rotation_matrix_2d(degree)
    ...
    def get_rotation_3d(self, vector_3: Vector3, rotation_vector_3: Vector3) -> Vector3:
        rotation_matrix_x = self.__get_rotation_matrix_3d(rotation_vector_3.x, AXIS.X)
        rotation_matrix_y = self.__get_rotation_matrix_3d(rotation_vector_3.y, AXIS.Y)
        rotation_matrix_z = self.__get_rotation_matrix_3d(rotation_vector_3.z, AXIS.Z)
